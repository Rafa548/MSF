...
print("Trabalho realizado pela Força de Resistência do ar no instante t = 0s: ", np.sum(arrayTrabalho[0:0]))

print("trabalho realizado pela Força de Resistência do ar no instante t = 0.4s: ", np.sum(arrayTrabalho[0:int(0.4/deltaT) + 1]))

print("trabalho realizado pela Força de Resistência do ar no instante t = 0.8s: ", np.sum(arrayTrabalho[0:int(0.8/deltaT) + 1]))


# Integral
print("Integral 0s: ", 0)

print("Integral 0.4s: ", deltaT * ((arrayFunc[0] + arrayFunc[int(0.4/deltaT)]) * 0.5 + np.sum(arrayFunc[1:int(0.4/deltaT)])))

print("Integral 0.8s: ", deltaT * ((arrayFunc[0] + arrayFunc[int(0.8/deltaT)]) * 0.5 + np.sum(arrayFunc[1:int(0.8/deltaT)])))
