...
# Este exercicio é igual ao 13 mas tenho de considerar o peso no eixo X e a normal também mudou
...
arrayX[0] = 0
arrayVeloX[0] = 1
arrayAcelX[0] = ((potenciaCiclista / (arrayVeloX[0])) - (0.5 * densidadeAr * area * cres * (arrayVeloX[0] ** 2)) - (u * m * g * np.cos(angulo)) - (m * g * np.sin(angulo)))/m
...
for i in range(1, n + 1):
    ...
    arrayAcelX[i] = ((potenciaCiclista / (arrayVeloX[i])) - (0.5 * densidadeAr * area * cres * (arrayVeloX[i] ** 2)) - (u * m * g * np.cos(angulo)) - (m * g * np.sin(angulo)))/m
